...
class CredentialSigner {
  ...
  /**
   * Signe une presentation request en JWT (OpenID4VP)
   * @param {Object} presentationRequest - Les données de la présentation request
   * @returns {string} JWT signé
   */
  signPresentationRequest(presentationRequest) {
    const now = Math.floor(Date.now() / 1000);

    // Header avec ES256 (EC signature)
    const header = {
      alg: 'ES256',
      typ: 'openid4vp+jwt',
      kid: 'ec-key-1',
    ...
    // Le payload contient exactement les données OpenID4VP
    const payload = {
      ...presentationRequest,
      iat: now,
      exp: now + 600, // 10 minutes d'expiration
    };

    // Signer avec la clé EC privée
    const token = jwt.sign(payload, this.keyManager.getECPrivateKey(), {
      algorithm: 'ES256',
      header,
      noTimestamp: false,
    });
